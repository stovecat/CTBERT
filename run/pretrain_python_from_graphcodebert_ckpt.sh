...
cd ..
pip install --user transformers tree_sitter > log.txt 2>&1

MODEL_NAME=microsoft/graphcodebert-base
OUTPUT=../ckpt/pretrain/graphcodebert_python_from_graphcodebert_ckpt
...
CUDA_VISIBLE_DEVICES=$1 python -m torch.distributed.launch --nproc_per_node=${PER_NODE_GPU} --nnodes=${NUM_NODE} --node_rank=${NODE_INDEX} --master_port=2234 pretrain_python.py \
    --output_dir=$OUTPUT \
