...
OUTPUT=../ckpt/pretrain/codeidiombert_only_full_python_from_graphcodebert_ckpt
...
TRAIN_BATCH_SIZE=8 # per gpu batch
EVAL_BATCH_SIZE=16
ACCUMULATE_STEPS=4
LANGUAGE=python
LEARNING_RATE=2e-4
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=${PER_NODE_GPU} --nnodes=${NUM_NODE} --node_rank=${NODE_INDEX} --master_port=2234 pretrain_code_idiom_python.py \
    --output_dir=$OUTPUT \
