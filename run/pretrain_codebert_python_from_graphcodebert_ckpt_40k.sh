...
OUTPUT=../ckpt/pretrain/codebert_python_from_graphcodebert_ckpt_40k
...
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.launch --nproc_per_node=${PER_NODE_GPU} --nnodes=${NUM_NODE} --node_rank=${NODE_INDEX} --master_port=2234 pretrain_python.py \
    --output_dir=$OUTPUT \
    ...
    --seed 123456 \
    --fp16 \
    --not_use_dfg 2>&1| tee $OUTPUT/train.log
