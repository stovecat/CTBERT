...
OUTPUT=../ckpt/pretrain/graphcodebert_balance
...
EVAL_BATCH_SIZE=32
ACCUMULATE_STEPS=2
LANGUAGE=ruby,python,java,php,go,javascript
LEARNING_RATE=2e-4
WEIGHT_DECAY=0.01
ADAM_EPS=1e-6
MAX_SREPS=200000
...
python -m torch.distributed.launch --nproc_per_node=${PER_NODE_GPU} --nnodes=${NUM_NODE} --node_rank=${NODE_INDEX} --master_port=2234 pretrain.py \
    --output_dir=$OUTPUT \
