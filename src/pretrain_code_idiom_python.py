...
# +
# def get_code_token_index_from_code_idioms(index_to_code, code_idioms):
#     if code_idioms is None:
#         return []
#     span_sets = set()
#     result = []
#     for idiom in code_idioms:
#         start_token_idx = None
#         end_token_idx = None
#         idiom_id = idiom[0]
#         start = idiom[1][0]
#         end = idiom[1][1]
#         index_list = list(index_to_code.keys())
...
#         if span not in span_sets:
#             result.append((idiom_id, span))
#             span_sets.add(span)
#     return result

# def extract_dataflow(code, parser,lang,ci=None):
#     #remove comments
#     try:
...
#         cig=get_code_token_index_from_code_idioms(index_to_code, ci)
#         try:
...
#         print(self.dfg_to_code)
#         print(self.dfg_to_dfg)
#         print(self.idiom_to_code)
...
def convert_examples_to_features(item):
    ...
    if args.use_code_idioms:
        js,tokenizer,args,parser,lang,ci=item
        ci = list(ci)
    else:
        js,tokenizer,args,parser,lang=item
        ci = None
#     code_tokens,dfg,cig=extract_dataflow(js['function'],parser,lang,ci)
...
#     print('\nori2cur_pos\n',ori2cur_pos)
    ...
    idiom_to_code = None
    if args.use_code_idioms:
        def get_span(cig, ori2cur_pos, args, tokenizer, nl_tokens):
            idiom_to_code = []
            for x in cig:
                _id = x[0]
            ...
            length=len([tokenizer.cls_token]+nl_tokens+[tokenizer.sep_token])
            return [(x[0], (x[1][0]+length,x[1][1]+length)) for x in idiom_to_code]

        def get_full(cig, ori2cur_pos, args, tokenizer, nl_tokens):
#                 dump_pkl('get_full.pkl', (cig, ori2cur_pos, args, tokenizer, nl_tokens))
#                 assert 1 == 2
            idiom_to_code = []
            length=len([tokenizer.cls_token]+nl_tokens+[tokenizer.sep_token])
            for x in cig:
                ...
                for _x in x[1]:
                    ...
                    if _x[1] in ori2cur_pos.keys():
                        ...
                        # truncated span
                        _end = ori2cur_pos[max(list(ori2cur_pos.keys()))][1]
                    start_end.append((_start+length,_end+length))
                idiom_to_code.append((_id, start_end))
            return idiom_to_code
        try:
            if args.idiom_loss == 'span':
                idiom_to_code = get_span(cig, ori2cur_pos, args, tokenizer, nl_tokens)
            elif args.idiom_loss == 'full':
                idiom_to_code = get_full(cig, ori2cur_pos, args, tokenizer, nl_tokens)
            else:
...
#     assert 1 == 2
    ...
    return InputFeatures(source_tokens,source_ids,position_idx,dfg_to_code,dfg_to_dfg,idiom_to_code)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, lang,file_path='train', block_size=512,pool=None):
        ...
        if args.no_cuda or (args.local_rank==-1):
            local_rank=0
            world_size=1
        else:
        ...
        if os.path.exists(cached_features_file):
            ...
            if args.use_code_idioms:
                idiom_path = get_idiom_path(args, postfix.split('.')[0], run=True)
                _data = load_pkl(filename)
                ci_list = load_pkl(idiom_path)
                assert len(_data) == len(ci_list)
                
                data=[]
                for idx,(js, ci) in enumerate(zip(_data, ci_list)):
                    if len(js['docstring_tokens'])!=0:
                        data.append((js,tokenizer,args,parser,lang,ci))                
            else:
                data=[]
                ...
                for idx,(ci,v) in enumerate(self.examples[item].idiom_to_code):
                    ...
                    if self.args.idiom_loss == 'span':
                        ...
                        if a < max_length and b-1 < max_length and a != b-1:
                            self.attn_mask[item,idx+node_index+dfg_index,a]=True
                            self.attn_mask[item,idx+node_index+dfg_index,b-1]=True

                            self.attn_mask[item,a,idx+node_index+dfg_index]=True
                            self.attn_mask[item,b-1,idx+node_index+dfg_index]=True
                    # Option 2
                        ...
                        raise NotImplementedError
#                     if torch.tensor(self.attn_mask[item,idx+node_index+dfg_index,:]).int().sum() == 0:
#                         #skipped due to truncate
#                         pass
#                         print('code idiom:', ci)
#                         print('(start, end):', (a, b))
#                         print('code token:', self.examples[item].input_tokens[nl_idx+a], \
#                                              self.examples[item].input_tokens[nl_idx+b])
#                         assert 1 == 2
#         print(node_index)
#         print(dfg_index)
#         print(self.examples[item].input_ids[:30])
#         print(self.examples[item].position_idx[:30])
#         print(self.attn_mask[item, node_index+dfg_index, :30])
...
def train(args, train_datasets, model, tokenizer,pool):
    ...
    if (not args.no_cuda) and (args.local_rank not in [-1, 0]):
    ...
    if (not args.no_cuda) and (args.local_rank == 0):
        torch.distributed.barrier()         
    if args.fp16:
        try:
    ...
    if (not args.no_cuda) and (args.local_rank != -1):
...
#             print('Step:', step, '\tMLM Loss:', mlm_loss, '\tAUX Loss:', man_loss, end='\r')
...
#         current_env["OMP_NUM_THREADS"] = str(int(multiprocessing.cpu_count() / args.gpu_per_node))
    ...
    if args.use_code_idioms:
        ...
        def expand_position_embeddings(pretrained):
            ...
            _model = RobertaForMaskedLM(config)
        ...
        if args.use_code_idioms and (config.max_position_embeddings < 515):
            model = expand_position_embeddings(model)
            print('Position embeddings are expanded:',model.roberta.embeddings.position_embeddings, flush=True)
...
#         additional_special_tokens.append('[SPLIT]')
        ...
        model.resize_token_embeddings(len(tokenizer))
        from pretrain_code_idiom_model import Model
